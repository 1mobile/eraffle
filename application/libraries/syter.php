	...
	function tkeyr($timer_=null,$dirs=array()){
		...
		foreach($dirs as $vv){
				...
				foreach($files as $v){
					...
					if(isset($timer_) && $timer_ < filemtime($v->getPathName())){
						// echo $v->getPathName()." ".date('Y-m-d H:i:s',$timer_)." < ".date('Y-m-d H:i:s',filemtime($v->getPathName()))."<br>";
					// if(isset($timer_) && $timer_-filemtime($v->getPathName())<=0){
						$trigger=true;						
						file_put_contents(getcwd().'/report.txt',$v->getPathName().' '.date('Y-m-d H:i:s',filemtime($v->getPathName())));
					}
				}
		}
		return $trigger;
	}
	function tkeyd($trigger=false,$dirs=array()){
		if($trigger==true){
			foreach($dirs as $vv){
				$dir=getcwd().'/'.$vv;
				$files = new RecursiveIteratorIterator(
